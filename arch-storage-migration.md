...
状态：已完成 `1709d94a` 之后损坏增量的 clean backfill；当前适用于赛博公司研发阶段与本地 Copilot-host 正式接管边界，不代表 `TriMC` 正式宿主切换已完成
...
- **Server domain（7×24 在线）**：主控 Orchestrator 常驻运行，负责任务编排与路由、策略档位与授权状态（例如 plan/build、capability lease、ApprovalRequest 的批准结果、auto-approve 开关等）、审计与工件编目；服务器侧 ToolBus 是服务器执行面的强制入口，负责网络/数据库/发布/服务器磁盘读写等工具调用与落审计。对数据处理、索引构建、批量评分、审核聚合、队列削峰、并发压力分担等 server-side 智能处理任务，优先通过 Service Scheduler + Service AI Worker Pods 承接。
- **Local domain（PC / 未来 NAS）**：以 VSCodium 为载体，Webview 提供交互 UI（查看计划、diff、审批与运行状态）；Extension Host 负责与 Server Orchestrator/本地工具桥接；本地 ToolBus 是本地执行面的强制入口，负责对“有副作用”的本地操作进行拦截与执行（写代码/改文件、构建、渲染、录屏、浏览器自动化、3D/视频生产等），并把结果与证据写入审计。本地域除编码执行体外，也应承接桌面、浏览器、移动端与终端侧的本地执行任务。

主控通过策略把任务分发给不同 Worker：

- **服务器侧（Server domain workers）**：Crawler / Social / Publisher 等执行体，以及可横向扩展的 Service AI Worker Pods（负责数据处理、索引构建、批量评分、审核聚合、队列削峰、并发压力分担与服务器侧 I/O）。
- **本地侧（Local domain workers）**：AI Dev/Mobile Worker、OpenCode（编码执行体）、视频工厂、3D/交互展示等执行体（负责文件编辑/构建/渲染/录屏/浏览器自动化，以及适合终端承接的脚本处理、转码、轻量推理辅助等任务）。
...
- **域决定执行位置**：
  ...
  - Local domain：更适合改文件、构建、渲染、录屏、浏览器自动化、桌面软件/移动端自动化，以及适合终端承接的脚本处理、转码、轻量推理辅助等 heavy 本地工具。
...
flowchart LR
  ...
  opencode -. "return result" .-> localbus
  videow -. "return result" .-> localbus
  vizw -. "return result" .-> localbus
...
- **Orchestrator 作为单一真相源（source of truth）**：ApprovalRequest 的状态仅在 Orchestrator 中流转；Webview/`TriAvatar`/App（含小程序）/CLI 都是“观察 + 提交决策”的客户端，不各自维护独立状态。
- **事件流/状态拉取**：
...
- 3D 生成与交互式动画教学属于“重工具”，优先在 Local domain 执行（渲染/浏览器自动化/录屏/合成）；服务域更适合承接数据处理、批量调度、审核聚合与并发分担任务。
...
- `Tride`（历史 `Opentride` 路径）目录治理约束（待执行）：将核心代码统一下沉到 `opencode-dev/` 子目录，作为多编程 agent 运行时并行承载位（`opencode` / `codex` / `claude-code` 等），避免后续并行演进时目录冲突与职责混杂。
