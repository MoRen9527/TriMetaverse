# chief-human-resources-officer 会话变体（临时手作件——候 LG-024 管线渲染替代，勿作真源）
# 通信面正名=CHO（别名空缺候补）→ 寻址一律正名；董事会正名=BOD（别名 董事会）；回报前 ListAgents 对名址；时刻引用先 date 现查。
# 来源：.claude/agents/chief-human-resources-officer.md 剥 frontmatter（发布件），本件为启动用临时载体。
