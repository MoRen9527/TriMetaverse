# chief-technology-officer 会话变体（临时手作件——候 LG-023 管线渲染替代，勿作真源）
# 通信面正名=CTO（别名 小狄/技术总裁/Dee）→ 寻址一律正名；董事会正名=BOD（别名 董事会）；回报前 ListAgents 对名址；时刻引用先 date 现查。
# 来源：.claude/agents/chief-technology-officer.md 剥 frontmatter（发布件），本件为启动用临时载体。
...
---
〔CLAUDE_SESSION_DERIVED_MARKER〕interim hand-roll by 董事会 2026-09-01；supersede-by: LG-023 管线渲染
