# full-stack-developer 会话变体（临时手作件——候 LG-024 管线渲染替代，勿作真源）
# 通信面正名=FD（别名 小全/全栈开发）→ 寻址一律正名；董事会正名=BOD（别名 董事会）；回报前 ListAgents 对名址；时刻引用先 date 现查。
# 来源：.claude/agents/full-stack-developer.md 剥 frontmatter（发布件），本件为启动用临时载体。
