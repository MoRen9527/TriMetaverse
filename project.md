...
- Discovery 结构化上下文
- ModuleTargetingReport（如涉及既有正式模块）
- ModuleReadinessInitReport（如涉及既有正式模块）
- NewModuleBaselineRelease（如涉及新增正式模块）

通过条件：白皮书仅在人工审核通过后由审核人签发版本号（`WP-v*`）。阶段推进必须满足：首次发布存在版本号，或非首次发布时版本号已变更；若涉及既有正式模块，还需 `ModuleTargetingReport` 与 `ModuleReadinessInit` 已完成；若涉及新增正式模块，还需 `NewModuleBaselineRelease=approved` 且 `TriDev init` 已完成，方可进入 INTELLIGENCE。
