...
三元模块概念层对应二十个可部署工程模块，分四层推进：

| 层级 | 模块 | 阶段定位 | 商业角色 |
|------|------|----------|----------|
| **入口层** | TriGateway、TriAvatar、TriMobile、TriPilot + TriLC + TriCode + vscodium（PC 端打包） | L1 最小独立系统 | 用户触达与交互 |
| **服务域** | TriMC、TriStaciss、TriModel、TriSkill | L0 基础设施 → L1 | Agent runtime + 模型路由 + 技能 |
