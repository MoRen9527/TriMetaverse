...
**M/R 面语义矩阵（LG-031 定谳 2026-09-04）**：四格=远端 M（TriMMC，治理决策面，最高成熟）｜远端 R（TriRMC，执行自治+周迁移，中高）｜本地 R（TriRLC，执行+letters+TriModel，中）｜本地 M（TriMLC，通道 Profile 501，低）。**连接=星形**（本地任一→中央跨面，非面内——CEO 模型「R-R 相连」隐含 falsify）；治理流向（LG-031 终裁最终定谳）=**双向回流**（M→R 成熟经验吸收与 R→M 反向改进回流并列保留）；R 受限→M 审核（D-15 门）；M 成熟→方案指导 R（组长验收）；**审计条款=「M/R 面按承载语义连，非按面名连」（D-17 同册）**。**演进模式=M→R 师徒传承（LG-033 定谳 2026-09-05）**：M 面每成熟一步（如值班/交互双位互备体系）即沉淀为手册教予 R 面，R 面接手后 M 面腾手演进下一步；已兑现样板=周平面迁移（M 面先跑熟周平面工作流，方案移交河源 TriRMC 后每周日 23:00 自主执行）；元虚拟持续试验、元现实迭代落地，构成「试验—传承—超越」循环。**两维进化总纲（CEO 定谳 2026-09-08）**：横向=本地域经交互总结固化流程（FADE 实例）输送服务器域无人值守执行；纵向=M 面以成熟产品组合搭建实验方案、快速发挥优势并找到系统局限，R 面放大优势、破开局限——双轴交替构成公司进化主循环。
