...
**终态架构立法（LG-031 终裁）**：TriRMC+TriRLC=未来现实应用系统核心（R 面），未来可单独部署=**元现实**；M 面（TriMMC+TriMLC）=**元虚拟**——「元」字辈语义正式定义；三通道工程（LG-032）按「先接后关」推进。

**M/R 面语义矩阵（LG-031 定谳 2026-09-04）**：四格=远端 M（TriMMC，治理决策面，最高成熟）｜远端 R（TriRMC，执行自治+周迁移，中高）｜本地 R（TriRLC，执行+letters+TriModel，中）｜本地 M（TriMLC，通道 Profile 501，低）。**连接=星形**（本地任一→中央跨面，非面内——CEO 模型「R-R 相连」隐含 falsify）；治理流向（LG-031 终裁修正）=**M→R 单向吸收为主**（元现实不断从元虚拟吸收成熟经验方案），反向改进可有但非主要；R 受限→M 审核（D-15 门）；M 成熟→方案指导 R（组长验收）；**审计条款=「M/R 面按承载语义连，非按面名连」（D-17 同册）**。
