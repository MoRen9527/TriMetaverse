...
**M/R 面语义矩阵（LG-031 定谳 2026-09-04）**：四格=远端 M（TriMMC，治理决策面，最高成熟）｜远端 R（TriRMC，执行自治+周迁移，中高）｜本地 R（TriRLC，执行+letters+TriModel，中）｜本地 M（TriMLC，通道 Profile 501，低）。**连接=星形**（本地任一→中央跨面，非面内——CEO 模型「R-R 相连」隐含 falsify）；治理流向=R 受限→M 审核（D-15 门）+M 成熟→方案指导 R（组长验收）+双向回流+成熟度异质供给；**审计条款=「M/R 面按承载语义连，非按面名连」（D-17 同册）**。
