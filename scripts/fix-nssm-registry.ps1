...
& $NSSM stop TriLC 2>$null | Out-Null
Start-Sleep 3

# 直接写注册表（无引号 = 无截断）
Write-Host "[2/3] 写注册表（8.3 路径，无引号）..." -ForegroundColor Yellow
Set-ItemProperty -Path $regPath -Name Application -Value "C:\PROGRA~1\nodejs\node.exe"
Set-ItemProperty -Path $regPath -Name AppParameters -Value "C:\PROGRA~1\TriCade\RESOUR~1\app\tools\trilc\dist\cli.js run"
