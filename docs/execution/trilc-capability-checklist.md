...
- **收口门禁（v2026.W33.5 新增，2026-08-12 CEO 定案）**：每轮收口 = 判定 + 清单登记 + **push 三仓同步**三件事一起完成，缺一不算收口。push 动作由**编排层统一执行**（团队 agent 本地完成工作后 push 步骤执行不稳定——已观测两次：C12/C13 轮小狄、C8/C9 轮小贾；push 不稳定列为运营已知项，收口时编排层直接复核 `git ls-remote` 与舰队克隆状态）。
