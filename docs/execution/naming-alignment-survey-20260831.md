...
## 五、终态裁决与执行（2026-08-31 23:3x，CEO 裁决·董事会转投）

- **采 C 档双名并书+物理冻结**：叙事名=文档与 registry 正名；物理目录与 systemd units 永久冻结不改；alias 表=唯一映射真源；B 档项仅自然编辑窗顺手对齐不排停机窗。
- **决策记录**：对 08-24「物理旧名照用」旧决策的处置=**部分维持并永久化**（物理名照用从「过渡兼容」升格为「终态冻结」，正名权归叙事面）；取代理由=刚稳态的同步链回归风险>改名收益（勘验六面+A/B/C 分档论证）。
- **A 档即时执行完成**：alias 表（company-governance-state.md）加「2026-08-31 命名对齐决议」行；协议 §6.1 TriMLC 行重定义正文随 v2.2 微修同批落（TriMLC-Channel 行新增=LG-020 立法配套⑥闭环）。
- **LG-021 收口**。两件在途核销确认：提权注册命令原文+daemon.mode 字段口径核清均已于 23:26 回执先行交付（交错过河）。
