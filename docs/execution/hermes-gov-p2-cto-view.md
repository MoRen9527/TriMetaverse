...
**勘正对齐注（17:0x）**：本件原表述「schema=消化判据」按 CEO 勘正扩为「schema=消化管道」（§4.1 四段形态）；三选一判据降为消化段阀门。
...
### 4.1 schema 层技术定位（CEO 17:0x 勘正对齐版：schema=消化管道）

**CEO 勘正口径**：「schema 是需要 agent 接入消化之后，结构化输出对接如员工自己的 wiki，把零散的知识形成知识体系，**不是简单的三选一**」。

**消化管道完整形态（四段）**：
```
①接入 ──→ ②消化 ──→ ③结构化输出 ──→ ④wiki 对接
agent 接入   规则引擎+LLM     产物契约（五段）   员工 wiki 落页
inbox 源     三选一阀门       页模板变换        注入消费面衔接
```
- **本件 §4.2-4.3 的规则集/执行协议 = ②消化+③输出 段的技术实现**；§4.2 的 digest|reject|escalate 三选一 = **消化段的一个阀门**（入场分类阀），非全貌。
- **①接入段**：agent（席位会话）产出零散知识→结构化入 inbox（7 字段白名单已有）；接入触发=会话收口/事件（§4.3 已设计）。
- **④wiki 对接段**：结构化产物→员工 wiki 落页（注入页契约 §4.4）；本段与 v2.0 注入链路（boot 注入+消费记录）天然衔接——wiki 即注入面。
- **管道目标**：零散知识→成体系（CEO 原话）——「成体系」的技术判据=页间关系（同 target_page 模板归并+tags 聚类候设计窗），非只是单页质量。
