...
- **阶段二完成（2026-08-11，MVP 门禁 5/5）**：实现于 TriMC 仓（`src/orchestration/session-bridge.ts` + app.ts 三端点 + dispatchAsync 执行器 + task-controller 回写）；类型债清零（TS2339/TS2741 源码修复 2 处、noImplicitAny 放宽标注 agent-loop 遗留、TriModel/agent-core dist 构建）；补提交 `src/mirror/`（此前未跟踪，tasks/mirror 端点依赖）。实证：spawn → agentId；`GET /internal/v1/agents` 注册表（agentId/sessionId/name 映射）；`POST /internal/v1/agents/{sid}/message` → task completed + result 回写（8.2s）；e2e dispatch（6 步 trace 全绿，route 命中 chieftechnologyofficer IO:100%，executor 真实回复"e2eDispatch链路OK。"）；重启后 spawn+注册表+消息桥全链路复验通过。已知项：bg 会话继承 trimc.service cgroup（systemd control-group），TriMC restart 会连带杀会话——需重派（秒级），后续可独立 scope 管理。
...
## 五、k8s 扩展设计原则（2026-08-12 记档，不实现）

> CEO 定案方向，先记档不实现，作为将来 k8s 化的设计约束。

1. **双模 manifests**：所有模块（TriMC、TriLC 远程端、TriCode、Tristaciss 等）的 k8s 清单按"单节点可跑 + 多节点可扩展"双模设计——单节点时降副本（replicas 1、去 PDB/反亲和），多节点时零改造扩展。
2. **本地 PC TriLC 保持 Windows 原生**：不容器化（PS7/TriCade/MSI 优势不放弃）；TriLC 的容器化形态服务于**远程扩展节点**（其他 PC/服务器）。
3. **分布式算力方向**：服务域（TriMC 集群）+ 本地域（各 PC 原生 TriLC）+ 扩展节点（k3s agent 加入）组成统一算力池；k3s 的 server+agent 拓扑支持此方向。
4. **镜像与依赖统一管理**：构建产物/镜像在服务器统一入库（将来 registry），各节点拉取，避免每节点重复构建。
5. 此原则不改变当前部署形态（TriMC=systemd + postgres 容器，见 §三.7）。
