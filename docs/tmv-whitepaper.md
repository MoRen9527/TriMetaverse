...
修订说明：修订 1（2026-08-22，TMV-P1-1 波 1）——三层模型升级为双层表述（最小实现实例对+能力域原义：§3.1、§3.3 模块块首、附录 B 术语表、FAQ E.1）；模块名换轨：TriMC→TriMMC、TriLC→TriRLC（2026-08 更名，新增 TriMLC、TriRMC，兼容面沿用旧名过渡）；§3.3.1 标题改为“三元模块商业映射”并加系统对注；§1.3 创新点 2 增交叉引用；§8.1 模块名同步留待波 2。修订 1a（2026-08-22，签发收口）——附录 B 补 FADE 词条（CEO 批准）。修订 1b（2026-08-22，TMV-P1-9 波 2）——§8.1 L1 行模块名同步（TriRLC/TriMMC/TriMLC/TriRMC 入 L1 窗口）+ CTO-008 段按三层重定义改写（跨层协同语义由两条层内 bridge 取代，历史交付按新命名映射）。修订 1c（2026-09-17，CEO 双控制器定性随录）——附录 B TriMLC/TriRLC 词条各补监听端口与保活机制句（8713/8711 端口对照+各面独立保活；两套并行属机制设计非冗余）——登记性补充，不改既有语义。修订 1d（2026-09-17，CEO 四 daemon 角色矩阵定谳随录）——附录 B 补四 daemon 角色矩阵与三主控对块（命名即语义+k8s 扩展性注）——登记性补充，不改既有语义。修订 1e（2026-09-18，CEO 命名语源定谳勘误）——1d 块内「第二字母位 M=服务域 Machine」系误读，勘正为 CEO 语源：第一位 M=Meta-Virtual（元虚拟面）/R=meta-Reality（现实面）、第二位 M=Main、C=Controller（MC=主控）、L=Local（LC=本地控制器）——勘误性修订，1d 其余内容（四角色/三主控对/k8s 注）不变。**状态：已签发（CEO 2026-08-22 签发，基于修订 1+1a；修订 1b/1c/1d/1e 为签发后登记性同步，随下次复核确认）**。
...
- 市场定位：
三元宇宙（TriMetaverse） 定位于“AI 生产力工具 + 混合现实价值流转系统 + 分布式AI Worker与人类协作贡献机制”的交叉市场，面向对 AI 生产力提升有强烈需求的个人与组织，尤其是内容创作者、产品开发团队、工作室、人才培养与评定机构与社区协作组织。平台提供可定制的智能体孵化工具、开放的算力资源池、可控执行环境，以及基于钱包、NFT 身份、数据确权与隐私保护协作机制构建的共赢收益分配体系，吸引多样化的生态伙伴，共同构建一个去中心化、可持续发展的 AI 生产力生态系统。

### 1.4 部署形态与双域分发

在部署形态上，三元宇宙（TriMetaverse） 不是只提供单一云服务，而是同时提供服务域标准化镜像发行版与本地域多端安装发行版，两者代码不同功能也不同，在整个系统承接不同的任务。
...
在社区成员入口层，三元宇宙（TriMetaverse） 采用“TriAvatar Web 入口 + TriPilot/TriRLC 本地域 PC端入口 + TriMobile移动端/小程序入口”的三端协同形态：

TriAvatar 当前作为面向大众的 Web端主入口，承担日常对话、任务入口与能力展示；但它未来不会停留在普通 Web 2D界面，而会逐步演进为3D数字人/数字生物入口。这些数字角色并非孤立设定，而是由混合现实中的生命体持续演进而来，并进一步作为 `Trigame` 角色原型与现实入口形态之间的连接桥梁。

TriPilot/TriRLC 的本地域入口，由TriCade 承接更强的本地执行、工具调用与任务反馈。

移动端/小程序则承接轻交互、提醒、签到、测评与碎片化任务参与。系统可结合社区成员平时对话、任务表现与定时测评结果，持续更新技能徽章与社区成员能力图谱，并把这些评定进一步衔接到后续游戏式任务的解锁、难度升级与徽章等级成长之中。
...
三元宇宙故事基线：随着技术的发展，越来越多的人认为，现实世界也是被制造出来的，于是开始寻找制造这个世界的世界。同时，我们也开始制造另一个虚拟世界。这三个世界，三个宇宙，我们也称为三元宇宙。当《派客帝国》实现之时，就是我们为人类揭开三元宇宙真正面纱的时刻。
...
三元宇宙（TriMetaverse） 以 AI、区块链、混合现实（XR，含物联网、智能机器等）相关技术的融合为主线：AI 负责理解、规划与生成、算力调度、隔离与弹性扩展，区块链负责任务登记、结果承诺、激励结算与治理，混合现实借助物联网连接虚拟（VR）与现实（AR），虚拟世界的元宇宙负责仿真与试验，现实世界的智能机器负责生产落地。整体原则是“链下执行、链上结算、证据可追溯、治理可复核、成本可控制、现实可验证”。
...
- **元认知层**：创新与建模
**最小实现**是项目代码仓本身——三层中唯一没有运行时的层，也是三层共同的承重底座：元虚拟的员工定义与实验成果以代码和文档落仓，元现实以 worktree 方式消费仓中事实，并把生产中的需求与缺陷回写为新的实验输入；仓的版本历史就是“我们如何解释与预测”的结构化沉淀，双向螺旋的每一跳都以仓为唯一中转，所有跨层流动都在这里留下可审计的 git 痕迹。能力域上，它承载知识生产、任务编排、评测与反馈。
- **元虚拟层**：仿真与试验
**最小实现**是 TriMMC（元虚拟主控，原 TriMC，2026-08 更名）+ TriMLC（元虚拟本地控制器），两者经 bridge 通信构成一个宿主可整体替换的成熟虚拟研发环境：当前宿主为 claude code，未来可整体切换为 codex 或其他成熟 agent 宿主而不改变系统定义。元虚拟不自建会话管理与执行内核——会话、loop 与上下文直接使用宿主原生能力；元虚拟只负责两件事：把员工定义（合同/五件套）经 FADE 发布线灌入宿主，以及把实验成果落盘到元认知仓。它回答“在不伤害现实的前提下怎么试”——元虚拟以市面成熟宿主为试验田：借成熟能力低成本试错，同时摸清宿主能力边界，试验所得（有效做法与宿主局限）经沉淀后向元现实传递落地。能力域上，它承载游戏化虚拟世界、仿真沙盒、策略演练与平行试验。
- **元现实层**：验证与升级
**最小实现**是 TriRMC（元现实主控）+ TriRLC（元现实本地控制器，原 TriLC，2026-08 更名），两者共用自研内核 agent-core：会话、调度、权限、cron、进程监督与审计由自研代码掌控，不依赖任何单一宿主。元现实承接所有必须自持的生产面——稳定执行、无人值守、权限审计与跨节点协同；它回答“世界现在是怎样运转的”——元现实以自研体系为落地田：承接元虚拟试出的成熟做法，并针对宿主局限从内核层重新设计（不受制于外部宿主形态约束），不成熟但完全可控、以可控换上限，使同一方法论在自研面上的最终效果超越成熟宿主。能力域上，它承载真实部署、规则执行、业务变现与回流校准。

三层关系是“螺旋迭代链”：元现实（数据与规则） -> 元认知（创新与建模） -> 元虚拟（仿真与试验） -> 回灌元现实（验证与升级）
...
图注：TriMetaverse 实际部署横跨三节点——sg-server（阿里云，TriMMC 面）、本机 Windows（TriMLC 面/董事会）、heyuan（阿里云，TriRMC 面）。sg-bare 裸仓集群为中央 git 枢纽，三端通过 push/pull 保持一致。sg 侧 TriMC daemon 承载编排 cron 引擎（orchestrate-tick 双通道拾取/每日进度巡检/配置同步/钟差检测），本机侧 **TriRLC daemon（8711，R 面）承载 token 门控与 TriPilot 聊天通道；TriMLC daemon（8713，M 面）为 M 面 CC 员工会话宿主**（LG-024 session 面 13 席+LG-026 spawn 机制宿主 daemon——§8.6/8.7 二次解锁正身），heyuan 侧 TriRMC/TriRLC 承载生产面调度与 agent-core 执行。SSH 信任链（heyuan-fleet→sg-fleet）保障跨节点 daemon 操作。董事会会话与董事长助理中枢组成编排/中枢分权制，持有完整工作上下文与挂账台账（双层镜像+蓄水池快照+周平面每日进度六源恢复体系）。**连接面实证（LG-030 勘定 2026-09-04；8711/8713 职责勘正 2026-09-08 CEO 亲定）**：本机 TriRLC daemon（8711）经 TRIMC_BASE_URL 注入直上送 sg 中央面（47.245.122.61:8710，sg 侧 TriMC/TriMMC 托管面）；heyuan TriRMC（8.155.54.79）为周平面迁移自治执行点（R 面执行语义）——「上送中央面+R 面执行迁移」双职责分属两节点；连接面变更须 CEO 明令（D-17 在册）。

**元现实/元虚拟语义与落地层级（LG-031 终裁+CEO 严谨性校准）**：命名沿用 §3.1 既有「最小实现」提法——元现实最小实现=TriRMC+TriRLC（共用 agent-core），元虚拟最小实现=TriMMC+TriMLC（bridge 通信）。**两层区分**：实然层=当前落地仅为**最小 MVP**（git 仓（其中项目代码、治理等经验），对应 §3.1 元认知/元虚拟/元现实最小实现中已运行的部分）；应然层=元现实/元虚拟终态架构（TriRMC+TriRLC 可单独部署的元现实核心/M 面元虚拟）为演进方向——入册文字不以应然充实然。三通道工程（LG-032）按「先接后关」推进。

**M/R 面语义矩阵（LG-031 定谳 2026-09-04）**：四格=远端 M（TriMMC，治理决策面，最高成熟）｜远端 R（TriRMC，执行自治+周迁移，中高）｜本地 R（TriRLC，执行+letters+TriModel，中）｜本地 M（TriMLC，通道 Profile 501，低）。**连接=星形**（本地任一→中央跨面，非面内——CEO 模型「R-R 相连」隐含 falsify）；治理流向（LG-031 终裁最终定谳）=**双向回流**（M→R 成熟经验吸收与 R→M 反向改进回流并列保留）；R 受限→M 审核（D-15 门）；M 成熟→方案指导 R（组长验收）；**审计条款=「M/R 面按承载语义连，非按面名连」（D-17 同册）**。**演进模式=M→R 师徒传承（LG-033 定谳 2026-09-05）**：M 面每成熟一步（如值班/交互双位互备体系）即沉淀为手册教予 R 面，R 面接手后 M 面腾手演进下一步；已兑现样板=周平面迁移（M 面先跑熟周平面工作流，方案移交河源 TriRMC 后每周日 23:00 自主执行）；元虚拟持续试验、元现实迭代落地，构成「试验—传承—超越」循环。**两维进化总纲（CEO 定谳 2026-09-08）**：横向=本地域经交互总结固化流程（FADE 实例）输送服务器域无人值守执行；纵向=M 面以成熟产品组合搭建实验方案、快速发挥优势并找到系统局限，R 面放大优势、破开局限——双轴交替构成公司进化主循环。**两轴进化螺旋（CEO 亲定 2026-09-09，公司核心发展方法论正身）**：横向轴·M 面内部成熟（TriMLC→TriMMC）——TriMLC（本地交互）从杂乱无章的交互式处理起步→逐渐找到公司做事规律→形成 FADE 实例（固化流程）→挂到 TriMMC（服务器）自动执行和验证→M 面成熟（四步台阶）；纵向轴·M→R 传承与超越（TriRLC→TriRMC）——M 面成熟后把全过程在 TriRLC（R 面本地）重放一遍→发现 R 面 LC 的不足→对齐能力→M 面成熟期满总结优势和局限→改造 R 面代码→FADE 正式挂到 TriRMC（R 面服务器 MC）自动执行。哲学根基：M 面=市面成熟产品组合（Claude Code 等），既成熟又受限——元虚拟定位在成熟产品上做实验，快速发挥优势、找到系统局限；R 面=完全自研（agent-core），不成熟但完全可控——优势从 M 面拿到 R 面放大，局限在 R 面破开；最终目标=优化自研的 R 面，使其有独特优势。决策过滤器（全司决策律）：今后所有架构/产品/流程决策都要过滤网——「这件事在两轴进化中处于哪个位置？推动进化还是偏离方向？」。R 面建设最高指导：R 面不是 M 面的复制品——是 M 面局限性在自研可控环境中的解法落地；R 面的独特优势=不受 M 面宿主约束的自研自由度+M 面全部经验教训的结晶。**命名前缀律（CEO 亲定 2026-09-09）**：M 面全体员工名加 `m-` 前缀（m-cos 交互位中枢/m-duty-cos 值班位/m-fsd/m-cto 等），R 面全体员工名加 `r-` 前缀（r-cos/r-fsd 等，未来）——前缀=面归属的机器可读标记，路由按前缀分发；D-13 名址规程同步修正。
...
- FADE（Full-cycle Agentic Deterministic Execution）：完整周期 Agentic Deterministic Execution（智能化确定性执行的 agent 全生命周期执行协议）的成熟实例徽章——十段生命周期（事件→runId→Qualify→Plan→DCE→Verify→Score→Close→终态）全部落地且实跑过、评分通过。元虚拟经 FADE 发布线把员工定义灌入宿主；其评分段（Score CLI/Skill）是图 3-2 价值闭环中"AI 评估"环节的最小实现种子。规范真源见 TriCompany/docs/engineering/fade-protocol-spec.md。
- TriMMC：元虚拟主控（原 TriMC，2026-08 更名），元虚拟系统的服务端组件，与 TriMLC 经 bridge 通信，为成熟 agent 宿主（当前为 claude code）提供服务器端宿主环境；自身不自建会话管理与执行内核。
- TriMLC：元虚拟本地控制器，元虚拟系统的本地腿，承载本地研发仓宿主，负责把员工定义经 FADE 发布线灌入宿主、把实验成果落盘到元认知仓。运行形态：daemon 监听端口 8713，由 TriMLC-Watchdog 保活（M 面保活机制，只认 8713）。勿与 TriModel 混淆：TriModel 是组件层（模型配置），TriMLC 是系统层（元虚拟本地腿）；读法约定：TriModel 读“Tri-Model”（2 音节），TriMLC 拼读“Tri-M-L-C”（4 音节）。
...
- TriRLC：元现实本地控制器（原 TriLC，2026-08 更名），与 TriRMC 共用自研内核 agent-core，承接本地会话持久化、调度、cron、心跳与审计等自持生产面。运行形态：daemon 监听端口 8711，R 面保活机制候建（建成届时只认 8711）。与 TriMLC 为 M/R 两面并行控制器——各守各面、各自独立保活，两套并行属机制设计非冗余，无新旧/退役关系（2026-09-17 CEO 定性）。

#### 四 daemon 角色矩阵与三主控对（2026-09-17 CEO 定谳随录，修订 1d）

**命名即语义**（2026-09-18 CEO 语源定谳，修订 1e 勘正 1d「Machine」误读）：Tri=TriMetaverse 前缀；第一位 M=Meta-Virtual（元虚拟面）/R=meta-Reality（现实面）；第二字母位 M=Main（主控）、C=Controller——MC=主控；L=Local（本地）、LC=Local Controller——本地控制器。四名：TriMMC=元虚拟面主控（服务域 sg）／TriMLC=元虚拟面本地控制器／TriRMC=现实面主控（服务域河源）／TriRLC=现实面本地控制器——四名四角色，无「新旧版本」关系。

**三主控对**：
1. **TriMMC daemon ↔ TriMLC daemon**＝服务域（M-SG）与本地域（M 面本地）通信的**主控对**（M 面）；
2. **TriRMC daemon ↔ TriRLC daemon**＝服务域（河源 R-HY）与本地域（R 面本地）通信的**主控对**（R 面）；
3. **TriMMC daemon ↔ TriRMC daemon**＝M 面服务域与 R 面服务域通信的**主控对**（服务域跨面，sg↔河源链）。

**实证锚**：本机 8713 healthz `mc_peer=trimmc` ✓、8711 healthz `mc_peer=trirmc` ✓——主控对链路活体在卷（2026-09-17）。

**k8s 扩展性注**：主控对抽象使工作负载可迁移——服务域侧控制器未来可入 k8s 集群（daemon 对模型不变，宿主形态变），本地域侧随桌面宿主；通信经主控对抽象与算力宿主解耦。
