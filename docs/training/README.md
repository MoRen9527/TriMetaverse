...
- [四 daemon 角色矩阵导读（TriMMC/TriMLC/TriRMC/TriRLC）](four-daemon-matrix-guide.md)——静态架构第一站（四格/命名语源/端口/保活/常见误区，2026-09-18 定谳基线）
- [三角优化循环：本地研发仓 × TriRLC × TriMMC（四 daemon 矩阵现势版）](trimc-trilc-devrepo-triangle-loop.md)——循环动态（训练循环 v2.1 语义/双轨互促/自研循环）
...
- ~~TriMMC、TriMLC、TriRMC、TriRLC（四 daemon 矩阵）模块导读~~（已补 2026-09-25：[四 daemon 角色矩阵导读](four-daemon-matrix-guide.md) + [三角优化循环现势版](trimc-trilc-devrepo-triangle-loop.md)）；TriPilot、TriStaciss、TriAvatar 等模块导读仍待补
