...
- lastSyncedAt: 2026-09-20（§12 部署面与源码面的区分管理增补；CEO 定调，BOD 起草+CAO 归口审）
...
| 职责 | Owner |
| --- | --- |
| 模块产品事实与 PRD 归属；产品域收口职责与模块设计/归属方案主责 | CPO（小乔） |
| 模块代码事实与技术门禁 | CTO（小狄） |
| 公司治理、秘书处与行政制度 | CAO |
| 人力资源、岗位启用与交接 | CHO |
...
### 12.6 派工同步前置（2026-09-20 增补，CEO 20:11 问定谳；BOD 起草+CAO 归口审）

1. **派工方**：派工含代码工作面时，工单须附一句「先同步工作树（fetch+ff-only；有本地产出先 commit）」——树协议形态下即任务书标准字段（D-27 衔接）；
2. **承接席自验**：开工前两项自验=status 干净+落后清零，任一不满足先处理再开工；
3. **机制兜底**：防护式自动 ff 已上线（worktree-guarded-ff.sh，20 仓轮巡每小时 :40；**干净树才 ff，脏树/在编辑仓跳过留人工**——首跑实证 TMV 脏树被正确跳过）——自动 ff 只兜干净树，脏树场景全靠本条纪律。

> 归位注记：本条落 §12 全员适用条款而非 COS 席位纪律——派工面不止 COS（BOD 直派/席间互派同适用），单一真源防多头规定（CEO 20:11 问裁定）。
