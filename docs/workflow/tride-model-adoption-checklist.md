...
当前文件是 TriMetaverse 宿主适配与 PC 端软件接入清单的本地真源，只负责宿主改造边界、能力下沉和接入检查项，不承担 TriCompany 公司级 workflow 书面真源职责。

目标不是恢复旧的三主控命名，而是明确：

1. 哪些能力应下沉到 `TriModel` 与 PC 端软件
2. 哪些能力必须继续留在 `TriMetaverse/docs/workflow/`
3. 两者之间的接口应该如何定义

## 2. 结论先行

应由 `TriModel` 承接的是“宿主适配与切换配置层”。
...
- `TriModel` 提供宿主适配、切换配置、run 生命周期入口与宿主状态语义
...
## 4. 下沉到 TriModel 和 PC 端软件的能力

以下内容应由 `TriModel` 或它配置下的宿主运行层承接。
...
`TriModel` 需要维护或暴露 machine-readable 的宿主运行状态，而不是只依赖对话历史。
...
`TriModel` 配置下的宿主运行层需要实现与 `docs/runs/` 契约兼容的写入器。
...
## 6. TriMetaverse 与 TriModel 的接口面
...
3. 给 `TriModel` 增加最小 run-state 持久化与 `start/resume/continue/next` 入口
4. 由 `TriModel` 配置下的宿主实现 `docs/runs/` 兼容写入器
...
以下 8 项全部满足，才能认为 `TriModel` 已经接住宿主适配层：
