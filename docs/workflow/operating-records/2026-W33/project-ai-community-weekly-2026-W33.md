...
## 2.2 服务器舰队计划 M0-M3 历程归档（server-fleet-trilc-parity）

> 本节归档 TriCompany 服务器舰队计划（TriMC 部署服务器 + TriLC 追平能力）M0-M3 完整历程。归档时间 2026-08-14（CEO 指令）。

### 2.2.1 计划缘起

双源头谱系：`TriCompany/docs/engineering/trilc-trimc-runtime-parity.md` V1.1（架构决策 2026-08-07）→ W33 决策登记（CEO 定案 2026-08-11）。一句话谱系：**TriLC 本地执行面 + TriMC 服务器舰队面互为 fallback，能力验证期由 TriMC 审核、TriLC 受验，最终形态 = 单代码库双部署**。

### 2.2.2 里程碑时间线

| 里程碑 | 时间 | 关键成果 | 证据指针 |
| --- | --- | --- | --- |
| M0 环境 | 08-11 | 服务器裸仓 + 舰队工作克隆 + git 同步链路打通，12/12 | `docs/execution/server-fleet-m0.md` |
| M1 试点 | 08-11 | 舰队自由对话实测通过 + TriMC 编排 MVP（session-bridge + 3 端点 + dispatchAsync + task-controller 回写），门禁 5/5 | 本纪要决策登记 M1 段 |
| M2 能力验证 | 08-12 ~ 08-13 | 12 轮树（r1-r12）全部 APPROVE：§二 1-6 域 25/25 全勾 + C 层 M2 受验必需项全过；验证期捕获修复两起真生产缺陷 | `docs/execution/trilc-capability-checklist.md` v2026.W33.15 |
| M3 独立资格+生产双跑 | 08-13 ~ 08-14 | R1 合同真源统一（v3.0 单一 schema，14 份，10 项口径零迁移丢失）→ R2 部署就绪（心跳契约对称补全 + 产物 v0.4.4 + runbook）→ R3 运营日就绪（联通面 connected + 服务器 dist 生产形态）→ 正式运营日 08-14 | trees/r13-contract-convergence、r14-production-dualrun、r15-ops-day-ready |

### 2.2.3 关键数字

- 3 天 3 里程碑（M0-M3），本周 10 树 done，全程零回退
- M2 能力项 25/25 全勾 + C 层 M2 必需项全过；12 轮树全部 APPROVE
- 运营日实测：14 员工实载（count 14，hasSystemPrompt 全 true）、安装态 v0.4.4、trimc connected（degraded→connected 心跳闭环）

### 2.2.4 关键事件

1. **系统重启 + 树协议存档读档首战恢复**（08-13 05:38 意外重启）：按 treeId + nodeId 定位崩溃节点、读 checkpoint 从断点幂等续跑，现场零丢失——树执行协议的实战验证通过
2. **contracts 打包缺失缺陷**（r12 捕获）：打包链路从不复制 contracts，现役安装态 count:0；4.2 安装态并入 5.2 的决策直接回报——不并入即漏网
3. **心跳契约盲区**（r14 捕获）：TriMC 心跳回显式、markNodeUnknown 零调用方——"双向契约"实为单向；契约 v1.0 对称补全（stale 30s / degraded 宽松 180s / 2 次回归 known）
4. **count 0 排查**（r15）：根因 = 安装前旧进程未重启（CreationDate 11:05:39 vs 安装备份 11:08:25 时间戳证据），非安装缺陷；附带发现 RegRun/schtasks 双启动冲突并清理
5. **运营日 CEO 实测确认**（08-14）：重启命令块全通过，version 0.4.4 + count 14 + connected 恢复

### 2.2.5 遗留项

- M4（源码替换）deferred——CEO 另行启动
- push 积压（本地领先 5+ commits，git 写权限受限）+ git 版本化/CI 记 M3 后续
- 11 仓存量 dirty（O1 修复后暴露，重启前遗留）
- CARRY-001 TUI 链（含 ink-testing-library 债）；BUG-20260805-001 Read 工具

### 2.2.6 经验教训

1. **树协议存档读档是抗中断真机制**：checkpoint 三字段（progress/artifactCommit/resumePoint）+ git 触发交接，让系统级中断从事故降级为"读档继续"
2. **独立验证纪律**：执行-验证分离，每项声称都当可疑对象独立重测（第二方法交叉验证），r7-r15 九轮零漏检
3. **双仓写方向单主体**：本地 → 裸仓 → 舰队克隆单向写，服务器仓 = 审核面不直改——避免双写冲突
4. **收口门禁 v2 四件事**：判定 + 登记 + push 同步 + 最终报告，缺一不算收口；agent 不回报由编排层按清单状态代收口
5. **正式安装态验证的价值**：源码能跑 ≠ 安装态能跑（W30 教训）；4.2 并入 5.2 的一次安装一次验，捕获了模拟态验不出的打包链缺陷
