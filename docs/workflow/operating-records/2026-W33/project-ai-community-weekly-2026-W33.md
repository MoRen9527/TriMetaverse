...
## 2.2 本周实战经验提炼：AI 团队协作研发的五个有效机制

> 本节提炼 W33 服务器舰队计划（M0-M3）实战中验证有效的协作机制。完整历程已在 `docs/execution/server-fleet-trilc-parity-plan.md` + OP 2.3.0 + 本纪要决策登记，不重复。

### 2.2.1 树协议存档读档：系统中断从事故降级为"读档继续"

**现象**：08-13 凌晨系统意外重启，全部 agent 进程死亡，工作现场只剩 tree-op.json 节点状态与 checkpoint 三字段。
**结论**：按 treeId + nodeId 定位崩溃节点、读 checkpoint（progress/artifactCommit/resumePoint）原地续跑，现场零丢失。checkpoint 写得越勤，中断成本越低——树协议不是流程装饰，是抗中断真机制。

### 2.2.2 独立验证纪律：研发自证会系统性漏掉自己刚写的缺陷

**现象**：小柯独立重测多次抓出执行方自证会漏的问题——r10 的 O2 死 schema（零消费方却注释自称 shared）、r12 的 contracts 打包缺失（模拟态全过、安装态 count 0）、历轮数字声称与实测不符（D5/D6/D7/D8/D9 系列）。
**结论**：执行与验证分离 + "每项声称都当可疑对象验" + 第二方法交叉验证（git rev-list / git 历史取回 / 符号级 grep），是 AI agent 协作的最低质量地板。验证方给可采纳的正确值，不是给 PASS/FAIL 标签。

### 2.2.3 正式安装态验证的价值：模拟环境全过 ≠ 生产环境能跑

**现象**：v0.4.4 模拟安装态 14/14 全过，正式 Program Files 安装后 count 0——根因是安装前旧进程未重启（CreationDate 早于安装备份时间戳），旧代码驻留内存。
**结论**：验证环境层级要显式区分（源码态 / 模拟安装态 / 正式安装态），最后一层必须实测。排查时时间戳证据（进程创建时间 vs 安装时间）比配置检查更快定位根因。

### 2.2.4 收口门禁 v2：agent 回话不可靠时，以文档状态为准代收口

**现象**：多轮观察——团队 agent 收口不回报是常态（工作已完成、仅叙事缺失），等回话会无限期卡住流水线。
**结论**：收口 = 判定 + 登记 + push 同步 + 最终报告四件事；PASS 信号到达后 10 分钟内无正式报告，编排层直接按清单状态收口——判定与登记落进文档即视为事实发生。

### 2.2.5 双仓写方向单主体：多智能体 git 协作的防冲突底线

**现象**：本地多仓 + 服务器舰队仓并行推进时，push 权限反复受限，服务器 git 落后本地 3-4 commits 成常态。
**结论**：写方向单主体（本地 → 裸仓 → 舰队克隆，舰队只读审核）+ push 由编排层统一补（`git ls-remote` 复核），把权限受限从阻塞点变成批量待办，任务不中断。
...
- **M2（能力验证期）**：TriLC 在 TriMC 监督下按 `docs/execution/trilc-capability-checklist.md` 逐项覆盖（真实研发任务驱动，不设固定时长）；并轨 TriLC 底座版本登记（当前 2.1.88）+ 每 1-2 月 rebase 审计——**状态：完成（2026-08-13）——12 轮 done（R1 C12/C13 → R2 C8/C9 → R3 C10 → R4 C1/C15 → R5 C15v2+2.4+2.5 → R6 2.1/2.2 → R7 3.1/3.2 → R8 1.1-1.5 → R9 3.3/3.4/4.1 → R10 4.2/4.3/6.1/6.2 → R11 6.3/6.4 → R12 5.1-5.4），§二 1-6 域 25/25 全勾 + C 层 M2 受验必需项全过，checklist v2026.W33.15；计划级登记见 `docs/execution/server-fleet-trilc-parity-plan.md`**
- **M3（独立资格 + 生产双跑）**：能力清单全勾 + 舰队审核通过 → 生产仓 = TriLC + TriMC 互为 fallback，正式运营日——**状态：完成（2026-08-14 正式运营日）——R1 合同真源统一（v3.0，10 项口径零迁移丢失）→ R2 部署就绪（心跳契约对称补全 + 产物 v0.4.4 + runbook）→ R3 运营日就绪（联通面 connected + 服务器 dist 生产形态）→ CEO 重启确认（version 0.4.4 + count 14 + connected 恢复）**
...
## 决策登记（2026-08-13，CEO 批准）

**议题**：M4 时机判断与生产级开发期启动

**定案**：

1. **M4 源码替换：时机未到，保持 deferred**。定义确认（带精确化）：被替换对象 = 服务器上的官方 claude 运行时（TriMC 编排层保留，不被替换）；替换物按 parity V1.1 机制 = 共享 agent-core 构建的运行时（最终单代码库双部署）。启动条件六条见 `docs/execution/production-grade-development-plan.md` §十，待 CEO 另行启动。
2. **生产级开发期启动**：M3 完成后自动进入（与 M3 自动推进逻辑一致）。TriCade + TriLC + TriMC 开始生产级开发：TriLC 主力执行、TriMC 把关审核；问题回研发仓修改（TriMC 仓问题直接改 TriMC，跨仓问题改研发仓后经 TriMC 审核）；循环持续改善 TriCade + TriLC，直到 CEO 认定 TriLC + TriCade 可作为第一个正式生产级可用版本（建议届时命名 v1.0.0）。
3. **运行方案定案**（`docs/execution/production-grade-development-plan.md` v2026.W33.1，正式版）：双环模型（生产环 TriCade→TriLC 主力→TriMC 把关；研发环 问题→登记→归仓→修复→审核→发布→验证）+ 共享周工作平面写锁规则（树节点文件范围锁；OP/tree-op 编排层统一写；周平面文件维护权 TriMC 侧——方向例外，代码修改仍本地发起）+ 分支-标签-审核-兜底代码仓治理（feature 分支 + 版本 tags + 三副本灾备 + 研发仓小贾兜底）+ 渐进权责四阶段（全量把关→抽检过渡→例外管理→资格认定）+ 组件自身优化流（入口四来源→小贾台账→归仓路由表→改造护栏：小改自修+单票审核 / 中改两票兜底复核 / 大改完整研发环）+ 发布节律（按需 + 当周有改动至少一版）+ 每版常设门禁（5.x 生产链标准 + 独立复核）。
4. **C 层口径修正**：C2-C16 原「M3 生产必需/M4 替换必需」标签停用，全部归生产级开发期消化清单，按四阶段挂靠（阶段一 C4/C5/C7 → 阶段二 C2/C3/C11/C14 → 阶段三 C16 → 阶段四 C6）。
5. **三小贾收敛设计**（方案文档独立一节）：身份是数据、运行时可替换；合同单一真源（r13 已达成）；五件套 memory 统一落点 TriCompany 仓（待做，列消化项）；互斥上岗（树节点写锁，同一员工同一时刻一个 active 实例）；研发仓小贾 = 过渡兜底，随四阶段淡出。
6. **首树启动**：`prod-grade-1-trimc-weekly-cron`——TriMC scheduler 模块（对标 TriLC cron：croner + CronJob store + CLI），定时触发 weekly_plane_shift 五段链，在服务器舰队克隆执行周平面迁移。节点链：r-1 小狄技术方案 → r-2 小全实现（本地研发仓开发 TriMC 仓，push sg-server；TriLC 参与测试/CLI 段）→ r-3 小柯独立验证 → r-4 小贾收口。硬 deadline：周日 2026-08-16 23:59 前可触发 W33→W34 迁移（下周一 08-17 新平面激活）。归仓路由判定：TriMC 自身仓中等改造 → 研发环主力实现。
