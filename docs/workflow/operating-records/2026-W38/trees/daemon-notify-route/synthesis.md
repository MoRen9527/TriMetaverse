...
- **P2**：全网格任意席→任意席、read 回执、群发
- **★设计北极星（CEO 2026-09-17 23:3x 定调）**：「有了 4 daemon 通信，就不用所有都从输入框走了」——**daemon 路由升格为主通道，输入框通道降级为例外兜底**（仅 UI 级操作：mode 切换//model//exit 类席位自管理命令；消息类通信一律走网格）。实证需求=输入框通道四连败（中文乱码/回车吞噬/模式循环误切/焦点锁）+模式态易碎（manual 卡审批连环）。P2 全网格由此从"候观察"激活为**方向性目标**（真痛点在案：屏扫痛点的本质+远控四连败）
