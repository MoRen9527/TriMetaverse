# LG-032 案 a 切指操作手册（TriRLC→TriRMC 值切位）——FD 小全拟稿候 BOD 定窗
...
- 边界：本件=切指窗操作清单（候 BOD 定窗后执行）；LG-032 方案正身=docs/execution/lg032-three-channels.md；连接变更合法性锚=D-17（CEO 明令第二次确认点后关旧通道）+LG-030（只换值零改 env 名/代码字段）
