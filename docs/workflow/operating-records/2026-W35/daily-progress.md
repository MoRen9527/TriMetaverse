...
- registry：无变化
- LG-016 件 2 收口（heyuan TriCompany clone 自动拉取 job，15min cron 同构）——job tricompany-pull（id fba9d2c7-9f97-406b-b2be-17ae38e076ac，TriRMC cron API POST 注册，`*/15 * * * *` Asia/Shanghai，command=`cd /srv/fleet/TriCompany && git pull --ff-only origin dev`，cwd=/srv/fleet，timeoutMs=120000，无 runAs）；四项实施清单逐项落卷：①safe.directory 核查=clone 属主 fleet 与操作者一致，dubious ownership 不触发，无需登记（sg root-pull 场景不适用）②非零退出+留日志=cron 引擎 lastError+per-run 日志自带，手动 run 实证 runCount=1/lastRunStatus=ok/lastError=None/durMs=918ms/日志 fba9d2c7...13-58-02Z.log 落盘 ③失败告警通道=V1 呈现面（healthz cron 块 degraded/consecutiveFailures+lastError）+周检抽样，QQ/邮件类维持增强项 ④fleet 单身份=服务级 User=fleet（trirmc.service 实证）故 job 不带 runAs（非 root runuser 会炸）+core.sharedRepository 未设（单身份无共享写）；前置实测=fleet 手动 pull 一次成功（clone 从 08-28 滞后拉到最新含 hub-ledger-governance.md，GitHub 通道 heyuan 可达免凭据）；nextRunAtMs 引擎自算（cron-job-state-hygiene 达标）。
- LG-016 件 3 收口（RFACE 真源化治理注入，时间盒内 22:15）——TriRMC 569c61e+6a2a53a（本地 dev）：rmc_tick.py 新增 build_governance_injection()（从 TriCompany 真源 engineering-disciplines.md 程序化提取 D-04/D-01/D-10 三纪律全文，platforms=agent-core 过滤定案①；头部 TriCompany HEAD sha1-12 机器锚=CTO-2 件 5 比对锚格式；真源即模板 CTO-1 零独立渲染件；真源缺失降级警告行不阻塞 tick；--print-injection 验收断言=CPO-1）；CPO-2 清点=旧 RFACE_SYSTEM_PROMPT 手抄条目（执行规则/工具优先级/收尾/git 合同）全部保留、纯增量零取代；部署弧线=GitHub push Connection reset 残差→git format-patch scp+fleet apply 直部署（两 patch）→首测 sha1 锚 unknown（root 验证通道 dubious ownership 假象，生产 fleet 通道无此问题）+兼容修 stdout=PIPE→**fleet 生产通道终测 selfcheck PASS+TriCompany@2dc329ed3366 锚+三节全在 rc=0**；heyuan 部署态 fleet commit ade009a（sync 标注，GitHub 通后以 origin reset 对齐）。回执留痕：四项清单+清点表+锚格式全在 daily-progress/board-journal。
- registry：无变化
