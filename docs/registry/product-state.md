...
- `TriMetaverse` 是整个三元宇宙的总战略、总 workflow、总白皮书和虚拟公司规则中枢。
- 它不是首轮产品交付 runtime，而是其他模块的战略和治理真源。

## Current Product Scope

- 维护总商业模式、阶段门禁、虚拟公司角色边界、模块映射和工作流规范。
- 维护 `TriMC` 统一运行面、虚拟公司经营载体、`TriHost` 宿主适配层与 PC 端软件层的中央产品边界。
- 维护虚拟公司岗位设计、总助体验、记忆治理等对产品侧有持续影响的设计收口结论。
- 维护各项目代码仓库的产品侧文档基线，包括 `PROJECT.md`、`REQUIREMENTS.md`、产品版 `ROADMAP.md` 与产品版 `STATE.md` 的归属和边界。
- 维护各项目代码仓库的 `模块五层文档协同系统` 基线，要求新模块默认建立 `docs/product/`、`docs/engineering/`、`docs/execution/`、`docs/registry/`、`docs/workflow/` 五层目录，并允许低成熟模块先用占位文件标记待初始化。
...
- 已具备较完整的白皮书、workflow、virtual-company 和阶段相关文档。
- 已建立中央 `BusinessStrategy` agent 与首批 registry 工作层。
- 已将中央运行与宿主边界统一收口为 `TriMC` 统一运行面、虚拟公司经营载体、`TriHost` 宿主适配层，以及 `Tripilot + Tride + vscodium + CLI` 的 PC 端软件层。
...
- 已把 TriCompany 现行的 `docs/engineering/`、`docs/execution/`、`docs/product/`、`docs/registry/`、`docs/workflow/` 五层结构评估为可推广的 `模块五层文档协同系统` 默认结构，并要求后续新模块至少先建占位入口。
...
- 当前中央登记层要求所有运行边界统一映射为：`TriMC` 负责运行面，虚拟公司负责经营载体，`TriHost` 负责宿主适配，`Tride/Tripilot/vscodium` 归于 PC 端软件层。
...
- `../../project.md`
- `../../virtual-company.md`
- `../workflow/virtual-company-agent-roles.md`
- `../workflow/virtual-company-secretariat.md`
