...
本文件对上游真源不重述、只指针；项目流程口径见 [docs/project.md](project.md)（不重述）。引用本文件时不得把文末附录历史归档当现行口径（附录仅为 2026-03-24 V0.1 演进证据）。动态经营事实真源=operating-records 周面+registry state，本件不承载周度进度。
