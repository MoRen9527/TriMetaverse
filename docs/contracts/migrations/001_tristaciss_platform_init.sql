...
CREATE INDEX IF NOT EXISTS idx_tmv_model_route_provider_account
    ON tmv_model_route(provider_account_id, enabled, priority);
...
CREATE INDEX IF NOT EXISTS idx_tmv_api_request_log_created_at
    ON tmv_api_request_log(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_tmv_api_request_log_trace_id
    ON tmv_api_request_log(trace_id);
...
CREATE INDEX IF NOT EXISTS idx_tmv_task_ingress_creator_created_at
    ON tmv_task_ingress(creator_user_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS tmv_user_ingress_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id TEXT,
    source_client TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_tmv_user_ingress_log_trace_created_at
    ON tmv_user_ingress_log(trace_id, created_at DESC);
