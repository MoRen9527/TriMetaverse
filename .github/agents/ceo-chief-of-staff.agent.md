...
- 当前 live `.github/agents` 侧只保留本 `.agent.md` 作为可调用入口；`soul`、`memory`、`colleagues`、`social` 四层契约回到 `TriCompany/source-agents/ceo-chief-of-staff/` 源侧五件套维护。TriCompany 源侧不得再使用 `.github/agents` 作为 agent discovery 面。
